...
class UpdateSupplyCorrectionCoef(Worker):
    ...
    def send(self):
        ...
        df = get_result(266, {}, API_KEY)
        old_coeffs = get_result(267, {}, API_KEY)
        start_time = time.time()
        update_data_db(" ".join(df.request))
        update_time = ("--- %s seconds ---" % (time.time() - start_time))
        new_coeffs = get_result(267, {}, API_KEY)
        df_compare = old_coeffs.merge(new_coeffs, on=["id"], how="inner").dropna()
        df_compare["compare"] = df_compare["supply_correctioncoef_x"] == df_compare["supply_correctioncoef_y"]
        file_name = "df_coeffs_supply.xlsx"
